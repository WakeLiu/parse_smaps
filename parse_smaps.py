...
example1: parse_smaps.py /proc/12424/smaps
...
def main():
    ...
    if ps_name == "":
        ...
        try:
            pid = ''.join(filter(lambda x: x.isdigit(), str(check_output(["pidof", ps_name]))))
        except:
...
#        arr.sort(cmp=lambda x,y: cmp(sum(y[0]), sum(x[0])))
        arr.sort(key=cmp_to_key(lambda a, b: sum(a[0]) - sum(b[0])))
